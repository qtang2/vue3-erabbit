...
export default {
  setup (props, { emit }) {
    ...
    // loading is to record send request. because for the if check, once page load,filterData always has value, so cannot show if we send request or not
    ...
    watch(() => route.params.id, (newVal) => {
      if (newVal && route.path === `/category/sub/${newVal}`) {
        ...
        findSubCategoryFilter(route.params.id).then(data => {
          // need to add '全部' attri to each top filter
    ...
    const getFilterParams = () => {
      // attrs should be like [{size: 'big'},{color: blue}]
